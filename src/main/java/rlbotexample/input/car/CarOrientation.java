...
public class CarOrientation extends Rotator {
	...
	/**
	 * The direction the roof of the car is facing. (0, 0, 1) means the car is
	 * upright.
	 */
	...
	public CarOrientation(Vector3 noseVector, Vector3 roofVector, double pitch, double yaw, double roll) {
		super((float) pitch, (float) roll, (float) yaw);
	...
	public static CarOrientation fromFlatbuffer(PlayerInfo playerInfo) {
		return convert(playerInfo.physics().rotation().pitch(), playerInfo.physics().rotation().yaw(),
